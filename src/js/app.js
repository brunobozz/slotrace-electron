...
window.applyMainColor = function (hexColor) {
  ...
  const darken = (c, pct) =>
    Math.max(0, Math.min(255, Math.round(c * (1 - pct))));
...
window.t = function (key) {
  ...
  const strings =
    window.localeStrings[lang] || window.localeStrings["pt"] || {};
...
function handleRoute() {
  ...
  if (mainRoute === "registrations") {
    const registrationsElement = document.querySelector(
      "slotrace-registrations",
    );
    if (
      registrationsElement &&
      typeof registrationsElement.updateSubRoute === "function"
    ) {
  ...
  if (mainRoute === "settings") {
    ...
    if (
      settingsElement &&
      typeof settingsElement.updateSubRoute === "function"
    ) {
    ...
    window.electronAPI.serial.onData((data) => {
      if (typeof data === "string") {
        ...
        if (match) {
          ...
          window.dispatchEvent(
            new CustomEvent("serial-sensor-triggered", {
              detail: { lane },
            }),
          );
        }
