class SlotRaceRegistrationsRacesCard extends HTMLElement {
      ...
      this.addEventListener("click", () => {
        if (this._race) {
          window.dispatchEvent(
            new CustomEvent("requestEditRaceName", {
              detail: { race: this._race },
            }),
          );
        }
      });
  ...
  render() {
    ...
    const trackName =
      race.trackName ||
      window.t("registrations.default_track") ||
      "Pista Padrão";
    ...
    this.innerHTML = `
      <div class="card h-100 bg-body-tertiary overflow-hidden">
        ...
        <div class="rounded-0 overflow-hidden bg-body-secondary w-100 position-relative" style="aspect-ratio: 16/9;">
          ${
            this._trackPhoto
              ? `
...
customElements.define(
  "slotrace-registrations-races-card",
  SlotRaceRegistrationsRacesCard,
);
