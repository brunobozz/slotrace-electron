class SlotRaceRegistrationsTracksCard extends HTMLElement {
  ...
  render() {
    ...
    const getShortLanesLabel = () => {
      const tVal =
        window.t("registrations.tracks_modal.lanes_label") || "Lanes";
      if (tVal.includes("Número de Fendas")) return "Fendas";
    ...
    const getShortLengthLabel = () => {
      const tVal =
        window.t("registrations.tracks_modal.length_label") || "Length";
    ...
    const getShortPowerbarsLabel = () => {
      const tVal =
        window.t("registrations.tracks_modal.powerbars_label") || "Power Zones";
      if (tVal.includes("Alimentação") || tVal.includes("Alimentación"))
        return "Zonas";
    ...
    if (track.recordTime) {
      ...
      const pilotName = recordPilot
        ? recordPilot.nickname || recordPilot.name
        : "";
      const pilotSuffix = pilotName ? ` (${pilotName})` : "";

      const recordLabelRaw =
        window.t("registrations.tracks_modal.record_pilot_label") ||
        "Record Pilot";
          ...
          <div class="d-flex align-items-center justify-content-between mt-auto pt-1">
            <div class="text-start overflow-hidden">
              <div class="small text-secondary text-truncate d-flex align-items-center" style="font-size: 0.8rem;">
                <span class="d-inline-flex align-items-center" title="${lanesLabel}">
                  <i class="mdi mdi-road-variant text-secondary-emphasis fs-6 me-1"></i>
                  <strong class="text-body-emphasis">${track.lanes || '-'}</strong>
                </span>
                <span class="mx-2 text-secondary opacity-50">|</span>
                <span class="d-inline-flex align-items-center" title="${lengthLabel}">
                  <i class="mdi mdi-ruler text-secondary-emphasis fs-6 me-1"></i>
                  <strong class="text-body-emphasis">${track.length ? track.length + 'm' : '-'}</strong>
                </span>
                <span class="mx-2 text-secondary opacity-50">|</span>
                <span class="d-inline-flex align-items-center" title="${powerbarsLabel}">
                  <i class="mdi mdi-flag-triangle text-secondary-emphasis fs-6 me-1"></i>
                  <strong class="text-body-emphasis">${track.powerbars || '-'}</strong>
                </span>
              </div>
              ${
                recordHtml
    ...
    if (editBtn) {
      editBtn.addEventListener("click", () => {
        window.dispatchEvent(
          new CustomEvent("requestEditTrack", {
            detail: { track },
          }),
        );
      });
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener("click", () => {
        window.dispatchEvent(
          new CustomEvent("requestDeleteTrack", {
...
customElements.define(
  "slotrace-registrations-tracks-card",
  SlotRaceRegistrationsTracksCard,
);
