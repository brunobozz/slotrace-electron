class SlotRaceRegistrationsTracksToolbar extends HTMLElement {
  connectedCallback() {
    ...
    this._langListener = () => {
      const searchVal = this.querySelector("#input-track-search")?.value || "";
      this.render();
      this.setupEvents();
      const input = this.querySelector("#input-track-search");
      if (input) {
    ...
    this.innerHTML = `
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
        ...
        <div class="d-flex align-items-center form-control form-control-lg search-wrapper-cardless border border-secondary-subtle rounded" style="max-width: 300px; width: 100%; background-color: #111111">
          ...
          <input type="text" id="input-track-search" class="form-control form-control-lg bg-transparent border-0 text-body-emphasis ps-1 pe-3 py-2 flex-grow-1" placeholder="${window.t("registrations.search_track_placeholder") || "Pesquisar pista..."}" style="outline: none; box-shadow: none; border: 0px !important;">
        </div>
      ...
      input.addEventListener("input", (e) => {
        ...
        debounceTimeout = setTimeout(() => {
          window.dispatchEvent(
            new CustomEvent("trackFilterChanged", {
