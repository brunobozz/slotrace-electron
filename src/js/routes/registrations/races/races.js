  ...
  setupEvents() {
    this._createEmptyRaceListener = () => {
      window.electronAPI.db.get('races').then(races => {
        ...
        const defaultTrack = window.t('registrations.default_track') || 'Pista Padrão';
    ...
    window.addEventListener('requestCreateEmptyRace', this._createEmptyRaceListener);
  }

  removeEvents() {
    if (this._createEmptyRaceListener) {
      window.removeEventListener('requestCreateEmptyRace', this._createEmptyRaceListener);
    }
  }
}
