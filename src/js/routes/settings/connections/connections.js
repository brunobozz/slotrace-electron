class SlotRaceSettingsConnections extends HTMLElement {
  ...
  async init() {
    ...
    if (window.electronAPI && window.electronAPI.serial) {
      ...
      this._statusCleanup = window.electronAPI.serial.onStatusChange((status) => {
        this.status = status;
  ...
  async refreshPorts() {
    ...
    try {
      ...
      if (refreshBtn) {
        ...
        refreshBtn.innerHTML = '<i class="mdi mdi-refresh me-1"></i>Atualizar Portas';
      }
    }
  }
  ...
  updatePortsSelect() {
    ...
    this.ports.forEach(port => {
      ...
      const friendlyName = port.friendlyName ? `${port.path} - ${port.friendlyName}` : port.path;
  ...
  updateStatusDisplay() {
    ...
    if (isConnected) {
      const modeText = this.status.path === 'SIMULAÇÃO' ? 'Conectado (Simulação)' : `Conectado na ${this.status.path}`;
      ...
      statusBadge.innerHTML = '<i class="mdi mdi-close-circle me-1"></i>Desconectado';
  ...
  setupFormEvents() {
    ...
    if (connectBtn) {
      connectBtn.addEventListener('click', async () => {
        ...
        this.addLog(`[CONEXÃO] Tentando conectar a ${path} a ${baudRate} bps...`);
        connectBtn.disabled = true;
  ...
  addLog(message) {
    ...
    logDiv.className = 'py-0.5 border-bottom border-secondary-subtle border-opacity-10';
      ...
      <div class="row g-3">
        ...
        <div class="col-md-5">
          <div class="card bg-body-tertiary border-secondary-subtle shadow-sm h-100">
            <div class="card-body">
              <h6 class="fw-bold mb-3 text-body-emphasis"><i class="mdi mdi-usb text-primary me-1.5"></i>Configurações do Sensor</h6>
              ...
              <div class="mb-3">
                ...
                <div class="d-flex gap-2">
                  ...
                  <button id="btn-refresh-ports" class="btn btn-secondary btn-sm flex-shrink-0 d-flex align-items-center" title="Atualizar Portas">
              ...
              <!-- Status badge -->
              <div class="mb-4 d-flex align-items-center justify-content-between py-2 border-top border-bottom border-secondary-subtle border-opacity-25">
                <span class="text-secondary small fw-semibold">Status de Conexão</span>
                <span id="status-badge" class="badge bg-danger px-3 py-2 fs-7"><i class="mdi mdi-close-circle me-1"></i>Desconectado</span>
              </div>
