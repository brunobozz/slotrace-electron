class SlotRaceRealtimeQuali extends HTMLElement {
  ...
  _startRunningLoop() {
    const tick = () => {
      ...
      if (this._state === 'qualifying' && this._firstLapMarked && this._lapStartTime && !this._pausedState) {
  ...
  _resetSession() {
    ...
    this._sessionConfig = {
      timePerPilot: this.race?.timePerPilot !== undefined ? this.race.timePerPilot : 60,
    ...
    this._pilotQueue = racePilots.map(p => typeof p === 'object' ? p.id : p);
  ...
  _handleStart(detail) {
    ...
    const allPilotIds = (this.race.pilots || []).map(p => typeof p === 'object' ? p.id : p);
  ...
  async _handleFinish() {
    ...
    window.dispatchEvent(new CustomEvent('raceQualiUpdated', { detail: { race: this.race } }));
  ...
  _updateDriverPanel() {
    ...
    if (this._currentPilotId) {
      ...
      panel.setData({ driver, qualiRecord: record, overallBestTime: overallBest, lane, laneColors });
    } else {
      panel.setData({ driver: null, qualiRecord: null, overallBestTime: 0, lane, laneColors });
    }
  }
  ...
  _updateLaps() {
    ...
    if (this._currentPilotId) {
      ...
      lapsEl.setData({ lapTimes: [], overallBestTime: overallBest, activeLapNum: 0, firstLapMarked: false });
    }
  }
  ...
  _updateStandings() {
    ...
    if (standings && this.race) {
      standings.setData({ quali: this.race.quali || [], drivers: this.drivers });
    }
  }
  ...
  _updateQueue() {
    ...
    const track = this.getTrackForRace();
    const laneColors = track ? track.laneColors : null;
    queue.setData({ pendingPilots, drivers: this.drivers, lane: this._sessionConfig.lane || 1, laneColors });
  }
  ...
  distributeData() {
    ...
    racePilots.forEach(pilot => {
      ...
      if (!exists) {
        this.race.quali.push({
          pilotId, laps: 0, bestLapIndex: 0, bestLapTime: 0, lapTimes: []
        });
      }
    });
    ...
    if (queue) {
      ...
      const track = this.getTrackForRace();
      const laneColors = track ? track.laneColors : null;
      queue.setData({ pendingPilots: allPilots, drivers: this.drivers, lane: this._sessionConfig.lane || 1, laneColors });
    }
  }
    ...
    this.innerHTML = `
      <div class="modal fade" id="modal-realtime-quali" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        <div class="modal-dialog modal-fullscreen">
          <div class="modal-content border-0 bg-dark text-white d-flex flex-column h-100" style="background-color: #0f1115 !important;">
            
            <!-- Header section -->
            <div class="modal-header border-bottom border-secondary-subtle bg-black bg-opacity-70 px-4 py-3 d-flex align-items-center justify-content-between" style="border-bottom-color: var(--bs-border-color-translucent) !important;">
              ...
              <div class="text-center flex-grow-1 mx-3">
                <h2 class="fw-bold text-white mb-0 text-uppercase tracking-wider" style="font-size: 1.5rem; letter-spacing: 0.05em; text-shadow: 0 2px 4px rgba(0,0,0,0.5);">
                  ${raceName}
                </h2>
              ...
              <div class="d-flex align-items-center gap-3">
                ...
                <button type="button" class="btn-close btn-close-white shadow-none" data-bs-dismiss="modal" aria-label="Close" style="outline: none; box-shadow: none;"></button>
              </div>
            ...
            <div class="modal-body flex-grow-1 p-0 d-flex overflow-hidden" style="background-color: #12151c !important;">
              
              <!-- Left Column: Driver Panel + Laps (side by side) -->
              <div class="d-flex flex-row h-100 flex-grow-1" style="background-color: #0f1115; border-right: 1px solid rgba(255,255,255,0.08);">
                <quali-driver-panel class="overflow-auto h-100" style="width: 50%;"></quali-driver-panel>
                <quali-laps class="overflow-auto h-100" style="width: 50%; border-left: 1px solid rgba(255,255,255,0.08);"></quali-laps>
              </div>
